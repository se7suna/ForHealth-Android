...
def get_recipe_image_storage_path() -> Path:
    ...
    """
    # 从配置中获取基础存储路径，然后拼接 recipe_images 子文件夹
    backend_dir = Path(__file__).parent.parent.parent
    base_path = Path(settings.IMAGE_STORAGE_PATH)
...
async def save_recipe_image(file: UploadFile, recipe_id: Optional[str] = None) -> str:
    ...
    if recipe_id:
        ...
        file_ext = Path(file.filename).suffix.lower() if file.filename else ".jpg"
        filename = f"{recipe_id}_{uuid.uuid4().hex[:8]}{file_ext}"
    else:
        # 否则使用UUID生成文件名
        file_ext = Path(file.filename).suffix.lower() if file.filename else ".jpg"
        filename = f"{uuid.uuid4().hex}{file_ext}"
...
def delete_recipe_image(image_url: Optional[str]) -> bool:
    ...
    try:
        ...
        if relative_path.startswith("recipe_images/"):
            storage_path = get_recipe_image_storage_path().parent  # 回到uploads目录
            file_path = storage_path / relative_path
        else:
