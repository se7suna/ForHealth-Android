...
async def get_nutrition_analysis(
    ...
    Returns:
        NutritionAnalysisResponse: 营养素分析结果
    """
    db = get_database()
...
async def get_time_series_trend(
    ...
    Args:
        ...
        start_date: 开始日期
        end_date: 结束日期
        view_type: 视图类型 (day, week, month)

    Returns:
        TimeSeriesTrendResponse: 时间序列趋势数据
    """
    db = get_database()
    ...
    if view_type not in ["day", "week", "month"]:
        raise ValueError("视图类型必须是 day, week 或 month")
...
async def export_health_report(
    ...
    Returns:
        HealthReportExportResponse: 完整的健康报告数据
    """
    db = get_database()
