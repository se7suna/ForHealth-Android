...
**`POST /api/food/barcode/recognize`** (**新增**)
- **功能**: 从图片中识别条形码数字
- **认证**: 需要登录（Bearer Token）
- **流程**:
  1. 接收上传的图片文件
  2. 使用 pyzbar 和 opencv-python 识别条形码
  3. 返回识别到的条形码数字和类型
- **响应**:
  - 成功: 返回条形码数字（如：6901939613702）和类型（如：EAN13）
  - 失败: 返回未识别提示
- **支持格式**:
  - 图片：JPG, PNG, BMP
  - 条形码：EAN-13, EAN-8, UPC-A, CODE128, QR Code 等
...
- `BarcodeImageRecognitionResponse` - 条形码图片识别响应模型
...
**`backend/app/utils/barcode_scanner.py`** (新增模块)
- `decode_barcode_from_image()` - 从图片识别条形码
- 支持多种图像预处理方法（二值化、锐化等）
- 智能识别策略提高成功率

**依赖更新** (`backend/requirements.txt`)
- 添加 `pyzbar>=0.1.9` - 条形码识别核心库
- 添加 `opencv-python>=4.8.0` - 图像处理
- 添加 `Pillow>=10.0.0` - 图片读取
- 添加 `numpy>=1.24.0` - 数值计算
...
#### 1. `POST /api/food/barcode/recognize` (**新增**)
...
#### 2. `GET /api/food/barcode/{barcode}`
...
```javascript
// 1. 用户选择或拍摄图片
const imageFile = await selectImage(); // 或 capturePhoto()

// 2. 上传图片识别条形码
const formData = new FormData();
formData.append('file', imageFile);

const recognizeResponse = await fetch('/api/food/barcode/recognize', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`
  },
...
if (recognizeResult.success) {
  const barcode = recognizeResult.barcode;
  console.log(`识别到条形码: ${barcode} (${recognizeResult.barcode_type})`);
  ...
  // 识别失败
  alert(recognizeResult.message);
  // 可以让用户重新拍照或手动输入条形码
}
```

> **注意**：
> - `ensureLocalFood` 可以封装以下逻辑：如果 `food_data.source === "local"`，直接返回 `food_data.id`；若为 `external` 或 `boohee`，则调用 `POST /api/food/` 将该食品保存到库中，并返回新建的本地 ID。
> - 图片识别方式适合用户拍摄商品照片的场景，无需专门的条形码扫描器
...
- **Web**:
  - HTML5 `<input type="file" accept="image/*">` 
  - HTML5 `<input type="file" accept="image/*" capture="environment">` (直接调用相机)
...
1. ✅ **两种扫描方式**：
   ...
   - **图片上传识别**（后端已完成，开箱即用）
2. ✅ **智能查询策略**：本地优先 → 薄荷API
3. ✅ **数据缓存**：薄荷食物自动保存到本地（`created_by="all"`）
4. ✅ **图像优化**：多种预处理方法提高识别成功率

**用户需要做**：
1. ✅ 测试功能（推荐）
2. 💻 前端集成（按需）
   - 方式1：集成相机扫描库
   - 方式2：使用图片上传识别（更简单，后端已完成）
