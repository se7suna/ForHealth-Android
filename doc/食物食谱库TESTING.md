...
#### 3.8 通过名称搜索食谱ID
1. 点击 **`GET /api/recipe/search-id`**
2. 点击 **"Try it out"**
3. 设置参数：`keyword = 健康`, `limit = 10`
4. 点击 **"Execute"**
...
#### 3.5.1 创建食谱记录
1. 先获取一个食谱ID：`GET /api/recipe/search?keyword=健康早餐套餐`
2. 复制食谱的 `id`
3. 点击 **`POST /api/recipe/record`**
4. 点击 **"Try it out"**
5. 输入测试数据：
```json
{
  "recipe_id": "从上一步获取的食谱ID",
  "scale": 1.0,
...
- ✅ 返回 `201 Created`
- ✅ `batch_id` 存在（用于后续管理）
- ✅ `total_records` 等于食谱中的食物数量
- ✅ 返回所有食物记录的ID列表
- ✅ 总营养正确计算

#### 3.5.2 查询食谱记录（按日期）
1. 点击 **`GET /api/recipe/record`**
2. 点击 **"Try it out"**
3. 设置参数：
...
- ✅ 返回批次列表
- ✅ 每个批次包含食谱名称和总营养
- ✅ `total_nutrition` 汇总所有批次的营养
- ✅ 按时间倒序排列

#### 3.5.3 按餐次筛选食谱记录
1. 在 **`GET /api/recipe/record`** 中
2. 设置参数：`meal_type = 早餐`
3. 点击 **"Execute"**
...
#### 3.5.5 更新食谱记录
1. 复制上一步返回的 `batch_id`
2. 点击 **`PUT /api/recipe/record/{batch_id}`**
3. 输入 `batch_id`
4. 输入更新数据：
```json
{
  "recorded_at": "2025-11-13T18:00:00",
  "meal_type": "晚餐",
  "notes": "晚上才吃"
}
```
...
- ✅ 返回 `200 OK`
- ✅ 更新成功
- ✅ `updated_count` 等于该批次的记录数

#### 3.5.6 尝试更新 scale（应失败）
1. 点击 **`PUT /api/recipe/record/{batch_id}`**
2. 输入 `batch_id`
3. 输入包含 `scale` 的数据：
```json
{
...
**预期结果**：
- ✅ 返回 `422 Unprocessable Entity`（因为请求体中不再包含 scale 字段）
...
**预期结果**：
- ✅ `notes` 字段包含 `[来自食谱: 食谱名称]` 标记
- ✅ 包含 `recipe_record_batch_id` 字段

#### 3.5.8 删除食谱记录
1. 点击 **`DELETE /api/recipe/record/{batch_id}`**
2. 输入要删除的 `batch_id`
3. 点击 **"Execute"**
...
- [ ] 只能删除自己的数据
- [ ] 只能编辑自己的数据
- [ ] 公共食物（created_by="all"）所有人可见且只读
- [ ] 用户创建的食物只有创建者可见（私有）
- [ ] 用户B搜索时看不到用户A创建的食物
- [ ] 薄荷健康缓存的食物（created_by="all"）所有人可见
- [ ] 系统食谱（created_by="all"）所有人可见且只读
- [ ] 用户创建的食谱只有创建者可见（私有）
- [ ] 用户B无法查看用户A创建的食谱
- [ ] 用户B搜索时看不到用户A创建的食谱
