# 华为云服务器部署完整方案

## 📋 目录

- [一、服务器配置推荐](#一服务器配置推荐)
- [二、部署架构设计](#二部署架构设计)
- [三、CI/CD 自动部署流程](#三cicd-自动部署流程)
- [四、GitLab Runner 部署方案](#四gitlab-runner-部署方案)
- [五、详细部署步骤](#五详细部署步骤)
- [六、验证和测试](#六验证和测试)
- [七、常用运维命令](#七常用运维命令)
- [八、故障排查](#八故障排查)
- [九、成本估算](#九成本估算)

---

## 一、服务器配置推荐

### 🖥️ 推荐配置方案

#### 方案一：经济型（适合小团队开发测试）

- **规格**: 通用计算型 S6
- **配置**: 2核 4GB（s6.large.2）
- **系统盘**: 40GB 高IO
- **数据盘**: 50GB 高IO（存储数据库和日志）
- **操作系统**: Ubuntu 22.04 LTS Server 64bit
- **带宽**: 5Mbps 按需计费或包年包月
- **预估费用**: ¥150-200/月

#### 方案二：标准型（推荐，适合生产环境）⭐

- **规格**: 通用计算型 S6
- **配置**: 4核 8GB（s6.xlarge.2）
- **系统盘**: 40GB 超高IO SSD
- **数据盘**: 100GB 超高IO SSD
- **操作系统**: Ubuntu 22.04 LTS Server 64bit
- **带宽**: 10Mbps
- **预估费用**: ¥350-450/月

#### 方案三：高性能型（多人团队频繁构建）

- **规格**: 通用计算型 S6
- **配置**: 8核 16GB（s6.2xlarge.2）
- **系统盘**: 40GB 超高IO SSD
- **数据盘**: 200GB 超高IO SSD
- **操作系统**: Ubuntu 22.04 LTS Server 64bit
- **带宽**: 20Mbps
- **预估费用**: ¥700-850/月

### 💡 推荐理由

基于项目技术栈分析：

- **FastAPI + MongoDB** 后端架构
- **GitLab Runner** 需要运行 Docker 容器
- **CI/CD 流水线** 需要同时运行多个服务（MongoDB, MailHog）
- **团队协作** 需要稳定的构建环境

**推荐选择方案二（4核8GB）**，理由：

1. ✅ 足够运行 FastAPI + MongoDB + GitLab Runner + Docker
2. ✅ 可以同时跑 2-3 个 CI/CD 任务
3. ✅ 性价比高，满足生产环境需求
4. ✅ 预留扩展空间

---

## 二、部署架构设计

```
┌─────────────────────────────────────────────────────────┐
│           华为云服务器 (Ubuntu 22.04)                      │
├─────────────────────────────────────────────────────────┤
│                                                           │
│  ┌─────────────────────────────────────────────────┐    │
│  │  GitLab Runner (Docker Executor)                │    │
│  │  - 监听 GitLab 仓库                              │    │
│  │  - 自动执行 CI/CD Pipeline                       │    │
│  └─────────────────────────────────────────────────┘    │
│                                                           │
│  ┌─────────────────────────────────────────────────┐    │
│  │  生产环境 (Docker Compose)                       │    │
│  │  ┌──────────────┐  ┌──────────────┐            │    │
│  │  │ FastAPI App  │  │   MongoDB    │            │    │
│  │  │  (Uvicorn)   │  │   Database   │            │    │
│  │  │  Port: 8000  │  │  Port: 27017 │            │    │
│  │  └──────────────┘  └──────────────┘            │    │
│  │  ┌──────────────┐  ┌──────────────┐            │    │
│  │  │    Nginx     │  │   MailHog    │            │    │
│  │  │  Port: 80/443│  │ Port: 1025   │            │    │
│  │  │              │  │ Web: 8025    │            │    │
│  │  └──────────────┘  └──────────────┘            │    │
│  └─────────────────────────────────────────────────┘    │
│                                                           │
│  ┌─────────────────────────────────────────────────┐    │
│  │  监控 & 日志                                      │    │
│  │  - Docker logs                                   │    │
│  │  - GitLab Runner logs                           │    │
│  └─────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────┘
          ↑                           ↓
    GitLab Webhook          自动拉取代码部署
    (main 分支变更)
```

---

## 三、CI/CD 自动部署流程

### 工作流程

```
1. 开发者推送代码到 main 分支
   ↓
2. GitLab 触发 Webhook
   ↓
3. 服务器上的 GitLab Runner 自动执行 Pipeline:
   - test 阶段：单元测试 + 集成测试
   - build 阶段：构建应用
   - deploy 阶段：自动部署到生产环境 ⭐
   ↓
4. 部署成功后自动重启服务
```

### 更新 .gitlab-ci.yml（新增部署阶段）

在现有的 `.gitlab-ci.yml` 文件末尾添加以下内容：

```yaml
# ==================== Deploy Stage ====================

deploy:production:
  stage: deploy
  image: docker:latest
  services:
    - docker:dind
  variables:
    DOCKER_DRIVER: overlay2
    DOCKER_TLS_CERTDIR: "/certs"
  before_script:
    - apk add --no-cache openssh-client
    - eval $(ssh-agent -s)
    - echo "$SSH_PRIVATE_KEY" | tr -d '\r' | ssh-add -
    - mkdir -p ~/.ssh
    - chmod 700 ~/.ssh
    - ssh-keyscan -H $DEPLOY_SERVER_IP >> ~/.ssh/known_hosts
  script:
    - echo "🚀 开始部署到生产环境..."
    - |
      ssh $DEPLOY_USER@$DEPLOY_SERVER_IP << 'ENDSSH'
        cd /opt/for_health
        git pull origin main
        docker-compose -f docker-compose.prod.yml down
        docker-compose -f docker-compose.prod.yml up -d --build
        docker-compose -f docker-compose.prod.yml ps
        echo "✅ 部署完成！"
      ENDSSH
  only:
    - main
  when: manual  # 手动触发，确保安全
  tags:
    - docker
  environment:
    name: production
    url: http://124.70.161.90:8000
```

---

## 四、GitLab Runner 部署方案

### Runner 配置特点

- **类型**: Shared Runner（团队共享）
- **执行器**: Docker Executor
- **并发**: 2-3 个任务（根据服务器配置）
- **标签**: `docker`, `production`
- **特权模式**: 开启（支持 Docker-in-Docker）

### Runner 优势

1. ✅ **团队共享**：所有组员都可以使用这个 Runner
2. ✅ **隔离环境**：每次构建都是全新的 Docker 容器
3. ✅ **自动清理**：构建完成后自动清理临时文件
4. ✅ **稳定可控**：自建 Runner，不受 GitLab.com 限制

---

## 五、详细部署步骤

### 第一步：购买和初始化服务器

#### 1. 在华为云控制台购买 ECS 服务器

- **区域**：选择离您最近的区域（如华南-广州）
- **规格**：通用计算型 S6 4核8GB
- **镜像**：Ubuntu 22.04 LTS Server 64bit
- **网络**：VPC 默认配置
- **安全组**：开放端口 22, 80, 443, 8000, 8025
- **登录方式**：密钥对（更安全）或密码

**端口说明：**
- 22: SSH 登录
- 80/443: HTTP/HTTPS（Nginx）
- 8000: FastAPI 后端
- 8025: MailHog Web UI（查看邮件）

#### 2. 登录服务器

```bash
ssh root@your_server_ip
```

#### 3. 系统更新

```bash
apt update && apt upgrade -y
```

#### 4. 设置时区

```bash
timedatectl set-timezone Asia/Shanghai
```

#### 5. 创建部署用户

```bash
useradd -m -s /bin/bash deploy
usermod -aG sudo deploy
echo "deploy ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
```

---

### 第二步：安装基础环境

#### 1. 切换到 deploy 用户

```bash
su - deploy
```

#### 2. 安装 Docker

```bash
# 下载安装脚本
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# 将当前用户加入 docker 组
sudo usermod -aG docker deploy

# 重新登录使 docker 组生效
exit
su - deploy

# 验证 Docker 安装
docker --version
docker run hello-world
```

#### 3. 安装 Docker Compose

```bash
# 1. 更新索引
sudo apt update

# 2. 安装 compose 插件（注意是“docker-compose-plugin”）
sudo apt install -y docker-compose-plugin

# 3. 验证（注意命令是 docker compose，中间有空格）
docker compose version
```

#### 4. 安装 Git

```bash
sudo apt install -y git
```

#### 5. 配置 Docker 镜像加速

```bash
sudo mkdir -p /etc/docker
sudo tee /etc/docker/daemon.json <<-'EOF'
{
  "registry-mirrors": [
    "https://dockerproxy.com",
    "https://docker.m.daocloud.io",
    "https://hub-mirror.c.163.com"
  ],
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
    "max-file": "3"
  }
}
EOF

sudo systemctl daemon-reload
sudo systemctl restart docker
```

---

### 第三步：部署 GitLab Runner

#### 1. 下载 GitLab Runner

```bash
#备份原有源列表（可选）

sudo cp /etc/apt/sources.list /etc/apt/sources.list.bak
#添加清华源
 /etc/apt/sources.list 或使用如下命令追加：
echo "deb https://mirrors.tuna.tsinghua.edu.cn/gitlab-runner/ubuntu $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/gitlab-runner.list
#添加 GPG 公钥
curl -fsSL https://packages.gitlab.com/gpg.key | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/gitlab-runner.gpg
更新并安装
sudo apt update
sudo apt install gitlab-runner
```

#### 2. 验证安装

      
```bash
gitlab-runner --version
```

#### 3. 注册 Runner

首先在 GitLab 项目中获取 Registration Token：

- 项目 → Settings → CI/CD → Runners → Expand → Registration token

```bash
sudo gitlab-runner register

# 按提示输入：
# GitLab URL: https://gitlab.com/
# Registration token: [从 GitLab 项目获取]
# Description: for-health-production-runner
# Tags: docker,production
# Executor: docker
# Default Docker image: python:3.11
```

#### 4. 配置 Runner

编辑配置文件：

```bash
sudo nano /etc/gitlab-runner/config.toml
```

修改为以下内容：

```toml
concurrent = 2  # 允许并发执行 2 个任务

[[runners]]
  name = "for-health-production-runner"
  url = "https://gitlab.com/"
  token = "YOUR_RUNNER_TOKEN"
  executor = "docker"

  [runners.docker]
    tls_verify = false
    image = "python:3.11"
    privileged = true  # 允许 Docker-in-Docker
    disable_entrypoint_overwrite = false
    oom_kill_disable = false
    disable_cache = false
    volumes = ["/cache", "/var/run/docker.sock:/var/run/docker.sock"]
    shm_size = 0
    pull_policy = "if-not-present"

  [runners.cache]
    [runners.cache.s3]
    [runners.cache.gcs]
```

#### 5. 重启 Runner

```bash
sudo gitlab-runner restart
```

#### 6. 验证 Runner 状态

```bash
sudo gitlab-runner status
sudo gitlab-runner list
```

#### 7. 在 GitLab 项目中验证

- 项目 → Settings → CI/CD → Runners
- 应该看到绿色的 Runner

---

### 第四步：部署生产环境

#### 1. 创建项目目录

```bash
sudo mkdir -p /opt/for_health
sudo chown deploy:deploy /opt/for_health
cd /opt/for_health
```

#### 2. 克隆项目代码

```bash
# 使用 HTTPS（需要输入用户名和密码/token）
git clone https://gitlab.com/your-group/for_health.git .

# 或使用 Deploy Token（更安全，推荐）
# GitLab: 项目 -> Settings -> Repository -> Deploy Tokens
# 创建一个只读的 Deploy Token
git clone https://gitlab-deploy-token:YOUR_TOKEN@gitlab.com/your-group/for_health.git .
```

#### 3. 创建生产环境配置文件

```bash
cd /opt/for_health
nano .env.production
```

填入以下内容：

```bash
# 数据库配置
MONGODB_URL=mongodb://mongodb:27017
DATABASE_NAME=for_health_prod

# 安全配置
SECRET_KEY=your-super-secret-key-change-this-in-production-min-32-chars
ALGORITHM=HS256
ACCESS_TOKEN_EXPIRE_MINUTES=30

# 邮件配置（使用 MailHog 邮件服务器）⭐
SMTP_HOST=mailhog
SMTP_PORT=1025
SMTP_USER=
SMTP_PASSWORD=
SMTP_FROM_EMAIL=noreply@forhealth.com
SMTP_FROM_NAME=For Health

# 应用配置
DEBUG=False
```

**📧 邮件服务器说明：**

- **当前配置**：使用 MailHog（推荐用于测试/演示环境）
  - ✅ 无需配置真实邮件服务器
  - ✅ 提供 Web 界面查看邮件：`http://服务器IP:8025`
  - ✅ 部署简单，开箱即用

- **如需切换到真实邮件服务器**（正式上线时使用）：
  ```bash
  # Gmail 配置示例
  SMTP_HOST=smtp.gmail.com
  SMTP_PORT=587
  SMTP_USER=your-email@gmail.com
  SMTP_PASSWORD=your-app-password

  # QQ 邮箱配置示例
  SMTP_HOST=smtp.qq.com
  SMTP_PORT=587
  SMTP_USER=your-qq-email@qq.com
  SMTP_PASSWORD=your-authorization-code

  # 163 邮箱配置示例
  SMTP_HOST=smtp.163.com
  SMTP_PORT=465
  SMTP_USER=your-163-email@163.com
  SMTP_PASSWORD=your-authorization-code
  ```

#### 4. 创建生产环境 docker-compose 文件

```bash
nano docker-compose.prod.yml
```

填入以下内容：

```yaml
version: '3.8'

services:
  # FastAPI 后端应用
  backend:
    build:
      context: ./backend
      dockerfile: Dockerfile
    container_name: for_health_backend
    restart: always
    ports:
      - "8000:8000"
    env_file:
      - .env.production
    depends_on:
      - mongodb
      - mailhog
    networks:
      - for_health_network
    volumes:
      - ./backend/app:/app/app
    healthcheck:
      test: ["CMD", "curl", "-f", "http://localhost:8000/health"]
      interval: 30s
      timeout: 10s
      retries: 3

  # MongoDB 数据库
  mongodb:
    image: mongo:latest
    container_name: for_health_mongodb
    restart: always
    ports:
      - "27017:27017"
    volumes:
      - mongodb_data:/data/db
      - ./backup:/backup  # 备份目录
    networks:
      - for_health_network
    environment:
      MONGO_INITDB_DATABASE: for_health_prod

  # MailHog 邮件服务器
  mailhog:
    image: axllent/mailpit:latest
    container_name: for_health_mailhog
    restart: always
    ports:
      - "1025:1025"  # SMTP 端口
      - "8025:8025"  # Web UI 端口
    networks:
      - for_health_network
    environment:
      MP_SMTP_BIND_ADDR: 0.0.0.0:1025
      MP_UI_BIND_ADDR: 0.0.0.0:8025

  # Nginx 反向代理（可选）
  nginx:
    image: nginx:alpine
    container_name: for_health_nginx
    restart: always
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx/nginx.conf:/etc/nginx/nginx.conf:ro
      - ./nginx/ssl:/etc/nginx/ssl:ro
    depends_on:
      - backend
    networks:
      - for_health_network

volumes:
  mongodb_data:
    driver: local

networks:
  for_health_network:
    driver: bridge
```

#### 5. 创建 Dockerfile

```bash
nano backend/Dockerfile
```

填入以下内容：

```dockerfile
FROM python:3.11-slim

WORKDIR /app

# 配置国内镜像源（解决网络问题）
RUN sed -i 's/deb.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list.d/debian.sources && \
    sed -i 's/security.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list.d/debian.sources

# 安装系统依赖
RUN apt-get update && apt-get install -y \
    gcc \
    g++ \
    curl \
    libffi-dev \
    libssl-dev \
    libzbar0 \
    && rm -rf /var/lib/apt/lists/*

# 复制依赖文件
COPY requirements.txt .

# 安装 Python 依赖（使用清华源加速）
RUN pip install --no-cache-dir --upgrade pip -i https://pypi.tuna.tsinghua.edu.cn/simple && \
    pip install --no-cache-dir -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple

# 复制应用代码
COPY . .

# 暴露端口
EXPOSE 8000

# 健康检查
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD curl -f http://localhost:8000/health || exit 1

# 启动命令
CMD ["uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "4"]
```

#### 6. 创建 Nginx 配置

```bash
mkdir -p nginx
nano nginx/nginx.conf
```

填入以下内容：

```nginx
events {
    worker_connections 1024;
}

http {
    upstream backend {
        server backend:8000;
    }

    server {
        listen 80;
        server_name your-domain.com;  # 替换为您的域名或 IP

        client_max_body_size 10M;

        location / {
            proxy_pass http://backend;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
            proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
            proxy_set_header X-Forwarded-Proto $scheme;

            # WebSocket 支持
            proxy_http_version 1.1;
            proxy_set_header Upgrade $http_upgrade;
            proxy_set_header Connection "upgrade";
        }

        location /health {
            access_log off;
            proxy_pass http://backend/health;
        }
    }
}
```

#### 7. 添加健康检查端点到后端

编辑 `backend/app/main.py`，添加健康检查端点：

```python
from datetime import datetime

@app.get("/health")
async def health_check():
    """健康检查端点"""
    return {
        "status": "healthy",
        "timestamp": datetime.now().isoformat(),
        "service": "for_health_backend"
    }
```

#### 8. 启动生产环境

1. 正式启用
```bash
cd /opt/for_health
docker-compose -f docker-compose.yml up -d
```

2. 重新构建并启用
```bash
cd /opt/for_health
docker-compose -f docker-compose.yml up -d --build
```
 docker-compose -f docker-compose.yml down
  docker-compose -f docker-compose.yml build --no-cache backend
  docker-compose -f docker-compose.yml up -d
3. 停止服务
```bash
cd /opt/for_health
docker-compose -f docker-compose.yml down -d
```

#### 9. 查看日志

```bash
docker-compose -f docker-compose.yml logs -f
```

#### 10. 验证部署

```bash
# 检查健康状态
curl http://localhost:8000/health

# 访问 API 文档
curl http://localhost:8000/docs
```

---

### 第五步：配置自动部署

#### 1. 生成 SSH 密钥

在服务器上执行：

```bash
ssh-keygen -t rsa -b 4096 -C "gitlab-ci@forhealth" -f ~/.ssh/gitlab_deploy_key
```

#### 2. 添加公钥到服务器

```bash
cat ~/.ssh/gitlab_deploy_key.pub >> ~/.ssh/authorized_keys
```

#### 3. 将私钥添加到 GitLab CI/CD 变量

```bash
# 查看私钥内容
cat ~/.ssh/gitlab_deploy_key
```

在 GitLab 项目中添加 CI/CD 变量：

- 项目 → Settings → CI/CD → Variables → Add Variable
- **Key**: `SSH_PRIVATE_KEY`
- **Value**: [粘贴私钥内容]
- **Type**: File
- **Protected**: Yes
- **Masked**: No

#### 4. 添加其他 CI/CD 变量

在 GitLab 项目中添加以下变量：

| Key | Value | Type | Protected |
|-----|-------|------|-----------|
| `DEPLOY_SERVER_IP` | 您的服务器 IP | Variable | Yes |
| `DEPLOY_USER` | deploy | Variable | Yes |

#### 5. 创建部署脚本

```bash
nano /opt/for_health/deploy.sh
```

填入以下内容：

```bash
#!/bin/bash
set -e

echo "🚀 开始部署 For Health 后端..."

# 拉取最新代码
echo "📥 拉取最新代码..."
git pull origin main

# 重新构建并启动服务
echo "🏗️ 构建 Docker 镜像..."
docker-compose -f docker-compose.prod.yml build --no-cache

echo "🔄 重启服务..."
docker-compose -f docker-compose.prod.yml down
docker-compose -f docker-compose.prod.yml up -d

# 等待服务启动
echo "⏳ 等待服务启动..."
sleep 10

# 健康检查
echo "🏥 健康检查..."
if curl -f http://localhost:8000/health; then
    echo "✅ 部署成功！服务运行正常。"
else
    echo "❌ 部署失败！服务健康检查未通过。"
    docker-compose -f docker-compose.prod.yml logs backend
    exit 1
fi

# 清理旧镜像
echo "🧹 清理旧镜像..."
docker image prune -f

echo "🎉 部署完成！"
```

赋予执行权限：

```bash
chmod +x /opt/for_health/deploy.sh
```

#### 6. 更新 .gitlab-ci.yml

在项目的 `.gitlab-ci.yml` 文件中添加 `deploy` 阶段（参见第三章）。

---

### 第六步：配置监控和日志

#### 1. 创建日志目录

```bash
sudo mkdir -p /var/log/for_health
sudo chown deploy:deploy /var/log/for_health
```

#### 2. 创建监控脚本

```bash
nano /opt/for_health/monitor.sh
```

填入以下内容：

```bash
#!/bin/bash

# 检查服务状态
echo "=== Docker 容器状态 ==="
docker-compose -f /opt/for_health/docker-compose.prod.yml ps

echo -e "\n=== 后端服务健康检查 ==="
curl -s http://localhost:8000/health | python3 -m json.tool

echo -e "\n=== 磁盘使用情况 ==="
df -h | grep -E '^/dev/'

echo -e "\n=== 内存使用情况 ==="
free -h

echo -e "\n=== Docker 镜像大小 ==="
docker system df

echo -e "\n=== 最近的容器日志 ==="
docker-compose -f /opt/for_health/docker-compose.prod.yml logs --tail=50 backend
```

赋予执行权限：

```bash
chmod +x /opt/for_health/monitor.sh
```

#### 3. 设置定时监控（可选）

```bash
crontab -e
```

添加以下内容（每小时检查一次）：

```bash
0 * * * * /opt/for_health/monitor.sh >> /var/log/for_health/monitor.log 2>&1
```

---

### 第七步：配置备份

#### 1. 创建备份脚本

```bash
nano /opt/for_health/backup.sh
```

填入以下内容：

```bash
#!/bin/bash
set -e

BACKUP_DIR="/opt/for_health/backup"
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="mongodb_backup_${DATE}.tar.gz"

echo "🗄️ 开始备份 MongoDB 数据库..."

# 创建备份目录
mkdir -p $BACKUP_DIR

# 备份 MongoDB
docker exec for_health_mongodb mongodump \
    --out /backup/dump_${DATE} \
    --db for_health_prod

# 压缩备份
cd $BACKUP_DIR
tar -czf $BACKUP_FILE dump_${DATE}
rm -rf dump_${DATE}

echo "✅ 备份完成: $BACKUP_FILE"

# 保留最近 7 天的备份
find $BACKUP_DIR -name "mongodb_backup_*.tar.gz" -mtime +7 -delete

echo "🧹 清理完成，保留最近 7 天的备份"
```

赋予执行权限：

```bash
chmod +x /opt/for_health/backup.sh
```

#### 2. 设置定时备份

```bash
crontab -e
```

添加以下内容（每天凌晨 2 点备份）：

```bash
0 2 * * * /opt/for_health/backup.sh >> /var/log/for_health/backup.log 2>&1
```

---

### 第八步：安全加固

#### 1. 配置防火墙

```bash
sudo ufw enable
sudo ufw allow 22/tcp    # SSH
sudo ufw allow 80/tcp    # HTTP
sudo ufw allow 443/tcp   # HTTPS
sudo ufw allow 8025/tcp  # MailHog Web UI
sudo ufw status
```

#### 2. 配置 fail2ban（防止暴力破解）

```bash
sudo apt install -y fail2ban
sudo systemctl enable fail2ban
sudo systemctl start fail2ban
```

#### 3. 禁用 root SSH 登录

```bash
sudo nano /etc/ssh/sshd_config
```

找到并修改以下行：

```
PermitRootLogin no
```

重启 SSH 服务：

```bash
sudo systemctl restart sshd
```

#### 4. 设置自动安全更新

```bash
sudo apt install -y unattended-upgrades
sudo dpkg-reconfigure -plow unattended-upgrades
```

---

## 六、验证和测试

### 1. 验证 GitLab Runner

在 GitLab 项目中提交代码，触发 Pipeline：

```bash
git add .
git commit -m "test: 测试 CI/CD Pipeline"
git push origin feature-test
```

### 2. 验证自动部署

合并到 main 分支，在 GitLab Pipeline 页面手动触发 `deploy:production` 任务。

### 3. 验证生产环境

```bash
# 健康检查
curl http://your-server-ip:8000/health

# API 文档
curl http://your-server-ip:8000/docs

# MailHog Web UI（查看邮件）
# 在浏览器中访问：http://your-server-ip:8025
```

### 4. 验证 Runner 日志

```bash
sudo gitlab-runner --debug run
```

### 5. 查看 Docker 日志

```bash
docker-compose -f docker-compose.prod.yml logs -f
```

---

## 七、常用运维命令

### 服务管理

```bash
# 查看所有容器状态
docker ps -a

# 重启所有服务
cd /opt/for_health
docker-compose -f docker-compose.prod.yml restart

# 重启单个服务
docker-compose -f docker-compose.prod.yml restart backend

# 查看实时日志
docker-compose -f docker-compose.prod.yml logs -f backend
```

### GitLab Runner 管理

```bash
# 查看 Runner 状态
sudo gitlab-runner status

# 重启 Runner
sudo gitlab-runner restart

# 查看 Runner 日志
sudo journalctl -u gitlab-runner -f

# 验证 Runner 配置
sudo gitlab-runner verify
```

### 数据库管理

```bash
# 进入 MongoDB 容器
docker exec -it for_health_mongodb mongosh

# 手动备份
/opt/for_health/backup.sh

# 恢复备份
docker exec -it for_health_mongodb mongorestore /backup/dump_YYYYMMDD_HHMMSS
```

### 监控和调试

```bash
# 查看资源使用
docker stats

# 查看磁盘使用
df -h

# 清理 Docker 资源
docker system prune -a

# 查看系统日志
sudo journalctl -xe
```

---

## 八、故障排查

| 问题 | 可能原因 | 解决方案 |
|------|---------|---------|
| Pipeline 卡在 pending | Runner 未连接 | 检查 `sudo gitlab-runner status` |
| Docker 镜像拉取失败 | 网络问题 | 配置镜像加速器 |
| 容器启动失败 | 配置错误 | 检查 `.env.production` 和日志 |
| 无法访问服务 | 防火墙/端口 | 检查安全组和 ufw |
| MongoDB 连接失败 | 网络/配置 | 检查 docker-compose 网络配置 |
| Runner 执行失败 | 权限/配置问题 | 检查 Runner 配置和 Docker 权限 |
| 部署脚本失败 | SSH 权限 | 检查 SSH 密钥和 CI/CD 变量 |

### 常见问题详细解决方案

#### 问题 1：GitLab Runner 无法拉取镜像

```bash
# 解决方案：配置镜像加速器（已在第二步配置）
sudo systemctl restart docker
sudo gitlab-runner restart
```

#### 问题 2：容器无法访问外网

```bash
# 解决方案：检查 DNS 配置
sudo nano /etc/docker/daemon.json

# 添加 DNS 配置
{
  "dns": ["8.8.8.8", "114.114.114.114"]
}

sudo systemctl restart docker
```

#### 问题 3：磁盘空间不足

```bash
# 清理 Docker 资源
docker system prune -a --volumes

# 清理日志
sudo journalctl --vacuum-time=7d
```

---

## 九、成本估算

### 方案二配置（4核8GB）月度成本

| 项目 | 费用 |
|-----|------|
| ECS 服务器（4核8GB） | ¥280-350/月 |
| 公网带宽 10Mbps | ¥50-80/月 |
| 数据盘 100GB SSD | ¥20-30/月 |
| **总计** | **¥350-460/月** |

### 优化建议

- **包年包月**：购买包年包月可享受折扣（约 7-8 折）
- **学生认证**：学生认证可享受更多优惠
- **按需计费**：合理使用按需计费的带宽
- **预留实例**：长期使用可购买预留实例

### 年度成本估算

- **按需付费**：¥350-460/月 × 12 = ¥4,200-5,520/年
- **包年购买（8折）**：约 ¥3,360-4,416/年
- **学生优惠（5折）**：约 ¥2,100-2,760/年

---

## 十、后续优化建议

### 1. 性能优化

- 配置 Redis 缓存
- 启用 CDN 加速静态资源
- 数据库索引优化
- 启用 Nginx 缓存

### 2. 安全优化

- 配置 SSL 证书（Let's Encrypt）
- 启用 API 限流
- 配置 WAF（Web 应用防火墙）
- 定期安全扫描

### 3. 监控优化

- 部署 Prometheus + Grafana 监控
- 配置告警通知（邮件/钉钉/企业微信）
- 日志聚合分析（ELK Stack）

### 4. 备份优化

- 配置异地备份
- 自动上传到对象存储
- 定期恢复测试

---

## 附录

### A. 常用端口说明

| 端口 | 服务 | 说明 |
|------|------|------|
| 22 | SSH | 服务器登录 |
| 80 | HTTP | Web 访问 |
| 443 | HTTPS | 加密 Web 访问 |
| 8000 | FastAPI | 后端 API 服务 |
| 27017 | MongoDB | 数据库 |
| 1025 | MailHog SMTP | 邮件发送端口 |
| 8025 | MailHog Web | 邮件 Web 界面 |

### B. 环境变量说明

| 变量名 | 说明 | 示例 |
|--------|------|------|
| `MONGODB_URL` | MongoDB 连接地址 | `mongodb://mongodb:27017` |
| `DATABASE_NAME` | 数据库名称 | `for_health_prod` |
| `SECRET_KEY` | JWT 密钥 | 随机字符串（32位以上） |
| `SMTP_HOST` | 邮件服务器地址 | `mailhog`（MailHog）或 `smtp.gmail.com`（Gmail） |
| `SMTP_PORT` | 邮件服务器端口 | `1025`（MailHog）或 `587`（Gmail） |
| `SMTP_USER` | 邮件用户名 | MailHog 可为空，Gmail 需填写 |
| `SMTP_PASSWORD` | 邮件密码 | MailHog 可为空，Gmail 需应用专用密码 |

### C. 参考资料

- [GitLab Runner 官方文档](https://docs.gitlab.com/runner/)
- [Docker 官方文档](https://docs.docker.com/)
- [FastAPI 官方文档](https://fastapi.tiangolo.com/)
- [MongoDB 官方文档](https://www.mongodb.com/docs/)
- [Nginx 官方文档](https://nginx.org/en/docs/)

---

## 联系方式

如有问题，请联系：

- **项目**: for_health
- **GitLab**: https://gitlab.com/your-group/for_health
- **文档维护**: 开发团队

---

**最后更新时间**: 2025-11-15
