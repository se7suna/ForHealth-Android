...
```bash
# 方式1：使用初始化脚本（推荐，35种常用食物）
cd data_init
python init_database.py

# 注意：默认使用覆盖模式，会清空现有食物数据
# 运行时需要输入 'yes' 确认操作
...
**初始化脚本说明**：
- 📁 位置：`backend/data_init/init_database.py`
- 📦 数据集：`backend/data_init/initial_foods_dataset.json`（35种常用基础食物）
- 🔄 默认模式：覆盖模式（清空后重新初始化）
- ✅ 包含分类：主食、蛋类、奶制品、豆制品、肉类、水果、蔬菜、坚果
