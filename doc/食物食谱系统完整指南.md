...
方式2: 食谱记录    POST /api/recipe/record
```

> **提示**：如果需要一次记录多个食物，推荐使用食谱记录功能，它会自动为食谱中的所有食物创建记录。
...
**数据初始化会在服务启动时自动执行**，无需手动操作。

初始化内容包括：
- ✅ 初始化运动类型数据（从 `app.config.settings.DefaultSports`）
- ✅ 初始化默认用户（测试和管理员账户）
- ✅ 初始化食物数据（从 `backend/app/data_init/initial_foods_dataset.json` 加载）

**初始化位置**：`backend/app/data_init/init_dataset.py`

> **提示**：首次启动服务时，系统会自动完成所有初始化工作。
...
| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/api/food/` | 创建食物（支持图片上传） |
| GET | `/api/food/search` | 搜索食物（薄荷API） |
| GET | `/api/food/search-id` | 按名称搜索ID |
| GET | `/api/food/{food_id}` | 获取详情 |
| PUT | `/api/food/{food_id}` | 更新食物信息 |
| PUT | `/api/food/{food_id}/image` | 更新食物图片 |
...
```http
# 创建食物（使用 multipart/form-data 格式）
POST /api/food/
...
创建食物接口 `POST /api/food/` 使用 **multipart/form-data** 格式，支持图片文件上传：

**必填字段**：
- `name`: 食物名称（字符串）
- `serving_size`: 标准份量（数字，单位：克）
- `calories`: 卡路里（数字，千卡）
- `protein`: 蛋白质（数字，克）
- `carbohydrates`: 碳水化合物（数字，克）
- `fat`: 脂肪（数字，克）

**可选字段**：
- `category`: 食物分类（字符串）
- `serving_unit`: 份量单位（字符串，默认：克）
- `brand`: 品牌（字符串）
- `barcode`: 条形码（字符串）
- `fiber`: 膳食纤维（数字，克）
- `sugar`: 糖分（数字，克）
- `sodium`: 钠（数字，毫克）
- `image`: 图片文件（支持 jpg/jpeg/png/webp/gif，最大10MB）

**更新食物图片**：使用 `PUT /api/food/{food_id}/image` 端点单独更新图片。
...
### Q2: 如何一次记录多个食物？
**A**: 
推荐使用食谱记录功能 `POST /api/recipe/record`：
- 自动为食谱中的所有食物创建记录
- 统一管理（batch_id），便于批量操作
- 支持 scale 参数调整份量倍数
- 如果食物不在食谱中，可以多次调用单个记录接口 `POST /api/food/record`
